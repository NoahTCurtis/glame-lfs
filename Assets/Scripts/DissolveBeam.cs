...
public class DissolveBeam : MonoBehaviour
{
	...
	private float myScale { get {
			if (Game.Manager<MaterialManager>().LocalSpace)
				return (transform.lossyScale.x + transform.lossyScale.y + transform.lossyScale.z) / 3.0f;
			else return 1.0f;
		}
	}
	...
	private void SetDissolveMaterial()
	{
		...
		MaterialManager mm = Game.Manager<MaterialManager>();

		var mats = new Material[renderer.materials.Length];
		for (var i = 0; i < renderer.materials.Length; i++)
		{
			var newMat = mm.GetDissolveMaterial(renderer.materials[i], transform);
